# Data Model: Backtest Detail View
...
This document defines the Pydantic view models for the Backtest Detail View feature. These models transform database entities into presentation-ready structures for Jinja2 templates.

---

## Core Entities (from Feature Spec)

- **Backtest Run**: Single backtest execution with unique run_id, strategy, configuration, and status
- **Performance Metrics**: Calculated values including returns, risk measures, and trading statistics
- **Backtest Configuration**: Immutable snapshot of parameters used for the backtest

**Note**: Individual Trade Records are NOT currently persisted in the database. The spec's trade blotter requirement will be implemented as a summary view using aggregated metrics (MVP approach).
...
Represents a single metric with value, formatting, and tooltip information.
...
class MetricDisplayItem(BaseModel):
    ...
    Example:
        ...
        ...     is_favorable=True
        ... )
        >>> print(metric.color_class)  # "text-green-400"
    """
...
Organized collection of metrics by category.
...
### 3. ConfigurationSnapshot

Display model for backtest configuration parameters.
...
Aggregated trading statistics (MVP - no individual trades).
...
### 5. BacktestDetailView

Main view model combining all detail page data.
...
class BacktestDetailView(BaseModel):
    ...
    configuration: ConfigurationSnapshot = Field(
...
### From Database to View Models

```python
from src.db.models.backtest import BacktestRun, PerformanceMetrics

def build_metrics_panel(metrics: Optional[PerformanceMetrics]) -> Optional[MetricsPanel]:
    """
...
def build_configuration(run: BacktestRun) -> ConfigurationSnapshot:
    """
...
def build_trading_summary(metrics: Optional[PerformanceMetrics]) -> Optional[TradingSummary]:
    """
...
def to_detail_view(run: BacktestRun) -> BacktestDetailView:
    """
...
|-------|------|---------------|
| run_id | Valid UUID | "Invalid backtest identifier" |
| execution_status | "success" or "failed" | "Status must be success or failed" |
| initial_capital | > 0 | "Capital must be positive" |
| date range | end > start | "End date must be after start date" |
| total_trades | >= 0 | "Trade count cannot be negative" |
| win_rate | 0-1 range | "Win rate must be between 0 and 1" |

---

## State Transitions

Not applicable - backtest runs are immutable after creation. The only state change is deletion, which removes the entire record.

---

## Future Enhancements

1. **Individual Trade Records**: Add `trades` table to persist entry/exit details for full trade blotter
2. **Equity Curve Data**: Store time-series portfolio values for interactive charts
3. **Indicator Values**: Persist strategy indicator calculations for chart overlays

These enhancements would require database schema changes and are outside MVP scope.
