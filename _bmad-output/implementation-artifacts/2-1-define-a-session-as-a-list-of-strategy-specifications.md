...
Code review 2026-08-17 — three adversarial layers (Blind Hunter, Edge Case Hunter, Acceptance Auditor).
53 raw findings → 35 after dedup: 6 decisions, 15 patches, 9 deferred, 5 dismissed. Every finding below
was independently re-verified by executing it against the real collaborators before being recorded.

**All 6 decisions resolved 2026-08-18** (Allay, batch approval) → 17 patches, 11 deferred. The two that
became standalone code patches lead the patch list; the rest folded into the docstring rewrite, the
`from_overrides` patch, and two new deferrals.

- [x] [Review][Patch] **[from Decision 1]** Canonicalise bar types in `_resolve_bar_types` — store `str(bar_type).upper()` — today `subscription_bar_types` keys on `bar_type.upper()` but appends the **original** string, so when strategy A holds `aapl.nasdaq-1-MINUTE-LAST-EXTERNAL` and strategy B holds `AAPL.NASDAQ-...`, the lowercase form wins. Verified: `load_ids` becomes `frozenset({'aapl.nasdaq'})`. `live_market_data.py:199-204` names this exact consequence in its own comment — "leave `load_ids` carrying a lowercase id IBKR will not resolve — a silently dead subscription". **Resolution:** canonicalise at the helper, following the same store-canonical rule AC #2 already sets for `strategy_id` (two specs for the same thing must compare equal, or Epic 5's comparison reads noise). This also fixes the single-strategy lowercase case, which a property-only fix leaves open. Scoped to this story's helper — the shared `resolve_live_bar_types` fix stays deferred so Epic 1's verified live path is untouched. Add the differing-case and first-seen-order tests the property never had. [src/models/session.py:126,243-251]
- [x] [Review][Patch] **[from Decision 2]** Reject duplicate `strategy_id` within a `SessionSpec` — two `momentum` specs share `order_id_tag="002"`, and `trader.py:399-403` raises `RuntimeError("Already registered a strategy with ID ...")` at node start. This is the multi-strategy shape the story exists to enable, failing at its first real use. **Resolution:** refuse at config time with a message naming the limit ("two entries for the same strategy need distinct instance tags, which this phase does not model — see Story 2.5"), per the story's own Key Insight #2. Decisive factor: widening validation later is backward-compatible, narrowing is not — so a 4-line validator that can be relaxed once Story 2.5 defines instance tags beats speculatively adding a persisted field to a frozen model before the runner exists to validate its shape. [src/models/session.py:228-230]

- [x] [Review][Patch] Whitespace-only `strategy_id` bypasses every validator — the `return data` guard hands off to "the field constraint", but `min_length=1` accepts `"   "`. Verified: `StrategySpec(strategy_id='   ', parameters={'total':'garbage'}, bar_types=('not-a-bar-type','not-a-bar-type'))` **constructs successfully** — no registry lookup, no parameter normalisation, no bar-type validation. AC #2/#3/#7 are all void on this path. Also makes `default_factory=dict` reachable, the only path on which it is. [src/models/session.py:175-176]
- [x] [Review][Patch] `_resolve_bar_types` mishandles every non-list/tuple input shape — `list(raw)` runs before the callee's own guards. A bare string explodes into characters, producing the exact `"Cannot subscribe to bar type 'A'"` message that `live_market_data.py:147-153` was written to prevent (verified); a non-iterable escapes as raw `TypeError` (verified: `bar_types=5` → `TypeError: 'int' object is not iterable`, which pydantic does **not** convert); a `set` is accepted with hash-randomised order despite the docstring promising "in the order given". One fix — type-guard the input and drop the `list()` — closes all three. [src/models/session.py:126]
- [x] [Review][Patch] `extra` defaults to `ignore` — a mistyped model-level key is swallowed — verified: `StrategySpec(strategy_id='sma_crossover', parametrs={'fast_period':3}, ...)` yields `fast_period=10` with no error. A live session then runs parameters the operator never chose. `ConfigDict(frozen=True, extra="forbid")` on both models. [src/models/session.py:150,225]
- [x] [Review][Patch] `data.get("parameters") or {}` swallows falsy non-dict values — `parameters=[]`, `0`, `""`, `False` are all accepted as "all defaults", while a *truthy* wrong-typed value is correctly rejected. `"parameters": []` is a realistic serialisation artifact. Use an explicit `is None` check. [src/models/session.py:181]
- [x] [Review][Patch] `from_overrides` double-resolves the strategy id and rejects fuzzy ids the constructor accepts — `build_strategy_params` gates on `StrategyRegistry.exists()`, which lacks the hyphen/underscore-stripping branch `.get()` has. Verified: `sma-crossover` resolves via the constructor but raises `ValueError: Unsupported strategy type` via `from_overrides`. The docstring claims fuzzy support. Resolve once via `_lookup_strategy(...).name` and pass the canonical name to both. **Absorbs Decision 3:** also pre-check `definition.param_model is None` before calling `build_strategy_params` (today that path dies at `strategy_factory.py:344` with `AttributeError: 'NoneType' object has no attribute 'model_fields'`, making `_normalise_parameters`' own remediation message unreachable on the primary construction path) — reuse the same message. **Resolution on the exception type:** `from_overrides` is a factory, not a pydantic constructor, so it keeps raising `ValueError` for resolution failures and `ValidationError` for model-shape failures; synthesising a `ValidationError` via `from_exception_data` is fragile across pydantic versions for cosmetic consistency. Add the `Raises:` section stating both, and add the `from_overrides` failure-path tests the file has none of. Story 2.2's CLI must catch `(ValidationError, ValueError)` — recorded in deferred-work. [src/models/session.py:208-211]
- [x] [Review][Patch] Module docstring overstates the protection and asserts a Story 2.2 guarantee as present fact — the "Known, accepted limit" paragraph covers only in-place `parameters` mutation, omitting `model_copy` (which produces genuinely *invalid* specs, not merely edited ones); and "no code path writes back to a stored spec's JSONB column" describes code that does not exist yet, in the present tense. This is the Epic 1 retro's flagged recurring failure mode — documentation overstating protection in the safety-relevant direction. Also state that "JSON-losslessly round-trippable" holds for `model_dump_json`/`model_validate_json`, not `model_dump`. **Absorbs Decisions 4, 5 and 6** — the rewritten paragraph must disclose the full set of limits, each pinned by a test so it is legible and any future fix is a deliberate, test-visible act: (a) **not hashable** — `hash(spec)` raises `TypeError: unhashable type: 'dict'`; making `parameters` a hashable mapping would need a frozen-mapping type pydantic must serialise, putting at risk the `Decimal` round trip AC #5 turns on, and nothing hashes a spec today; (b) **`model_copy(update=...)` bypasses validation** — verified to yield a `SessionSpec` with zero strategies, violating AC #1; overriding `model_copy` here alone would diverge from every other model in `src/models/`, and honest disclosure is the Epic 1 retro's own remedy; (c) **`schema_version` is recorded, not enforced** — narrow the claim to "recorded so a future reader can branch on it", since no reader exists yet and a gate written now would guess at Story 2.2's read path. Note that patch P3 (`extra="forbid"`) supplies most of the intended gate behaviour for free: a row written by newer code with unknown fields then fails loudly instead of silently dropping them. Add the `schema_version` tests (default, `ge=1`) — it is currently the only field in either model with zero coverage. [src/models/session.py:3-23]
- [x] [Review][Patch] `sprint-status.yaml` claims AC #4 closes a still-open deferred-work item — the entry says AC #4 "closes" the `IBKRSettings.model_dump()` credential-leak item, but that item's trigger ("the moment Epic 2 persists session configuration") has not arrived, nothing persists anything yet, and `deferred-work.md:38-43` is untouched by this commit. The story's own Dev Notes said explicitly: "do not claim otherwise in the completion notes." [_bmad-output/implementation-artifacts/sprint-status.yaml]
- [x] [Review][Patch] Nested `ValidationError` repr inside another `ValidationError` — `f"...: {exc}"` interpolates pydantic's full multi-line block (header, `input_value=`, docs URL) into a `ValueError`, which pydantic then wraps and re-renders with its own header and second docs URL. Extract `exc.errors()` and format one line. [src/models/session.py:105]
- [x] [Review][Patch] TEST: bar-type refusal cases never assert the message AC #7 names — six parametrized ids promise six distinct rejection *reasons*; the assertion is a bare `pytest.raises(ValidationError)`. Replacing the resolver's message with `raise ValueError("bad")` keeps all 25 tests green, silently destroying the operator diagnostics AC #7 requires. [tests/unit/models/test_session_spec.py:281-283]
- [x] [Review][Patch] TEST: round-trip test cannot detect `Decimal` precision loss — `Decimal.__eq__` is numeric, so `Decimal("10.0") == Decimal("10")`, and the `type(...) is Decimal` check confirms the class, not the exponent. A round trip mangling the exponent passes both assertions. Assert `restored.model_dump_json() == spec.model_dump_json()`. (The round trip is in fact lossless today — verified across all 7 registered strategies — so this is a guard gap, not a live bug.) [tests/unit/models/test_session_spec.py:226-235]
- [x] [Review][Patch] TEST: credential test is vacuous without `.env`, skips username/port, never inspects `parameters` — `tws_account`/`tws_password` default to `""`, so `if value:` short-circuits in CI and on a fresh clone, leaving `ibkr_host` (the one non-secret) as the only value checked. AC #4 also names username and port, neither of which is tested. The field-name scan covers five declared fields but never the free-form `parameters` keys — the only place a secret could realistically land. [tests/unit/models/test_session_spec.py:159-178]
- [x] [Review][Patch] TEST: import-purity AST check passes for `src.*` imports that transitively load Nautilus — the test compares only the literal first path segment against six package names. Adding `from src.core.strategy_registry import StrategyRegistry` at top level **passes** while loading all of `nautilus_trader` (verified) — the exact cost AC #8 exists to prevent, since in this codebase the framework is always reached *through* a first-party module. A subprocess assertion is both stronger and passing today (verified: `import src.models` loads none of the six). [tests/unit/models/test_session_spec.py:292-302]
- [x] [Review][Patch] TEST: the `_settings_map` link is unprovable — settings values equal the param-model defaults — `fast_ema_period=10`/`slow_ema_period=20`/`portfolio_value=1000000`/`position_size_pct=10.0` are identical to `SMAParameters`' own defaults, so deleting the entire `_settings_map` step of `build_strategy_params` leaves the assertion passing byte-for-byte. AC #3 names a three-link chain; only two are pinned. Monkeypatch a settings value away from the default. [tests/unit/models/test_session_spec.py:108-120]
- [x] [Review][Patch] TEST: "the model never calls `get_settings()`" is enforced only by a one-off grep — AC #3's second clause has no test; the file already has an `ast` walk that could carry it at near-zero cost. As delivered, a future `settings = settings or get_settings()` would be caught by nothing in CI, lint, or hooks. [tests/unit/models/test_session_spec.py:292]
- [x] [Review][Patch] TEST: the `param_model is None` branch has no coverage — all 7 registered strategies have a param model, so the branch and its remediation message are never exercised. `StrategyRegistry.register()` makes `param_model` optional and 4 of the 7 live in the `custom/` submodule. A stub-definition test covers it in two lines. [src/models/session.py:96-101]

- [x] [Review][Defer] `build_strategy_params` silently drops unknown override keys [src/core/strategy_factory.py:344] — deferred, pre-existing; already recorded under "Deferred from: story-2.1", re-confirmed empirically by this review
- [x] [Review][Defer] `resolve_live_bar_types` accepts a lowercase instrument id [src/core/live_market_data.py:205] — deferred, pre-existing; it dedups on the upper-cased key but appends the original `BarType`, so a *single* strategy with a lowercase bar type also yields a lowercase `load_id`. This is the root the cross-strategy decision above sits on
- [x] [Review][Defer] `StrategyRegistry.clear()` permanently empties the registry [src/core/strategy_registry.py:271-288] — deferred, pre-existing; `clear()` resets `_discovered=False` but `importlib` returns cached modules, so no `@register_strategy` re-fires. Every valid id then fails with "Registered strategies: ." No current test calls it, but `make test-unit` runs xdist-parallel
- [x] [Review][Defer] A strategy module failing import with a non-`ImportError` escapes uncaught [src/core/strategy_registry.py:271-288] — deferred, pre-existing; `discover()` catches only `ImportError` and `_lookup_strategy` only `KeyError`, so a broken `custom/` submodule surfaces as an arbitrary exception from a pydantic constructor
- [x] [Review][Defer] Re-validating a persisted spec against today's param model is lossy and brittle [src/models/session.py:103] — deferred; a param field removed later is silently dropped from a *sealed* session's record, tightening any param constraint makes previously-sealed sessions unloadable, and a renamed strategy makes referencing rows permanently unloadable. Depends on Story 2.2/2.4's persistence design
- [x] [Review][Defer] `json.dumps(spec.model_dump())` raises on `Decimal` [src/models/session.py:103] — deferred; verified `TypeError: Object of type Decimal is not JSON serializable`. Story 2.2 must persist via `model_dump_json()` or `model_dump(mode="json")`, never bare `model_dump()`
- [x] [Review][Defer] Validator errors carry an empty `loc` [src/models/session.py:160] — deferred; a model-level validator attaches errors to the model root, so unknown strategy, bad parameters, and bad bar types are indistinguishable by location. A CLI mapping `err["loc"][0]` to a flag gets `IndexError`. For Story 2.2's CLI error handling
- [x] [Review][Defer] `SessionStatus` has no terminal failure state [src/models/session.py:46-50] — deferred; a crashed session and a cleanly stopped one both record `stopped`, so any comparison filtering on `stopped` mixes complete and truncated runs. AC #6 mandates exactly four values, so this is an epic-level question for Story 2.3's state machine
- [x] [Review][Defer] Unit-tier tests transitively load Nautilus [tests/unit/models/test_session_spec.py] — deferred; every spec construction triggers the lazy `live_market_data` import, contradicting the documented "unit = no Nautilus" tier rule. Tests pass and run in 0.82s, so this is a tier-policy call, not a defect
- [x] [Review][Defer] **[from Decision 5]** Override `model_copy` to re-validate [src/models/session.py:150,225] — deferred, by decision 2026-08-18; the hole is disclosed in the docstring and pinned by a test for now. Overriding `model_copy` in this model alone would diverge from every other pydantic model in `src/models/`, and no consumer derives a modified spec today. **Action for Story 2.2:** decide alongside the repository write path, where a real mutation path first exists
- [x] [Review][Defer] **[from Decision 6]** Add a `schema_version` read gate [src/models/session.py:227] — deferred, by decision 2026-08-18; the docstring is narrowed to what is true instead. A gate today has nothing to gate — no reader, no stored rows — and would guess at a read path Story 2.2 has not defined. Patch P3 (`extra="forbid"`) supplies most of the intended behaviour meanwhile. **Action for Story 2.2:** define the gate when the read path lands

**Dismissed as noise (5):** ambiguous fuzzy-match raising a non-`KeyError` (refuted — `get()` raises only `KeyError` on the miss path, and no alias shadows a canonical name across all 28 entries); `tuple`-has-no-append tests "assert only Python semantics" (refuted — the mutation check reproduces a failing test under a `list` field, so the guard is load-bearing); `SessionStatus("created") is CREATED` tautology (harmless redundancy; `test_str_renders_the_bare_value` is the real discriminator and exists); "two construction paths yield different defaults" (measured identical, and the split is AC #3's design); "the `.upper()` dedup key is dead code" (refuted — it is the real-bug horn, promoted to a decision above).
