...
Code review 2026-08-30, three adversarial layers (Blind Hunter diff-only · Edge Case Hunter with
wheel access · Acceptance Auditor with spec). 22 findings after dedup; 6 dismissed. Three layer
claims were **refuted** by direct wheel reads and are recorded here so they are not re-raised:
`OrderAccepted`/`OrderFilled` both declare `VenueOrderId venue_order_id not None` in their ctors
(`model/events/order.pyx`), so the unguarded `str(event.venue_order_id)` can never render `"None"`;
live order events are published from a single `_evt_queue` asyncio task
(`live/execution_engine.py:130,410`), so `_orders` has no cross-thread race; and
`OrderEmulated`/`OrderReleased`/`OrderPendingUpdate`/`OrderPendingCancel` are unreachable here
(emulation unused; the IBKR adapter has `# TODO: self.generate_order_pending_cancel`,
`execution.py:878`).

- [x] [Review][Decision] **A duplicate `OrderFilled` that Nautilus itself refused to apply is still published, and the observer double-counts it** [src/core/live_order_path.py:483-484] — verified in the wheel, not inferred. `ExecutionEngine._apply_event_to_order` catches `KeyError` under the comment *"Protection against duplicate fills"* and `InvalidStateTrigger` for a fill the FSM rejects, logs, and returns (`execution/engine.pyx:1357-1369`) — but the `_msgbus.publish_c(topic=f"events.order.{event.strategy_id}")` at `:1174-1177` sits **outside** that guard and runs anyway. `Order.apply` is the raise site (`model/orders/base.pyx:1073`, `Condition.not_in(event.trade_id, self._trade_ids, ...)`). So Nautilus's own duplicate-fill protection is invisible downstream: any consumer summing `last_qty` off the event stream diverges from the order's real `filled_qty`. The observer reads `trade_id` at `:494` and logs it but never dedups on it. Reachable via IBKR re-delivering an `execDetails` for a seen `execId` — the adapter's single-instrument fill path has no dedup (`adapters/interactive_brokers/execution.py:1098-1104`; the only `execId` dedup is the spread path at `:1140-1148`), and `client/wrapper.py:452` documents that `execDetails` fires on `reqExecutions()` as well as on execution. Outcome: a 100-share order filled once but reported twice logs `cum_qty="200"` against `order_qty="100"`, then `>=` at `:504` deletes the entry, so a genuine third fill re-seeds at zero and logs `cum_qty` equal to that fill alone with `order_qty` absent. Decide: dedupe on `trade_id` per entry (memory cost, and a redelivered fill then emits no record at all), or emit the record but skip the accumulate, or accept the divergence and say so in the docstring. Found by two layers independently. **DECIDED 2026-08-30 — dedupe and emit a marked record:** track a `trade_ids` set per entry; on a repeat, do **not** accumulate, but still emit `order.filled` with `duplicate=True` and `cum_qty` unchanged. Rationale: `cum_qty` stays correct *and* the transcript keeps the evidence that the venue redelivered, which is what NFR21 exists for; a silent drop would make the redelivery unobservable. The set is bounded by fills-per-order. Now a `[Review][Patch]`.
- [x] [Review][Decision] **`OrderUpdated` is unhandled, so the harvested `order_qty` goes stale and completion detection breaks in both directions** [src/core/live_order_path.py:357-366, 504-505] — `order_qty` is written once by `_harvest_initialized` and never refreshed, but `OrderUpdated` carries the order's *current* quantity (`model/events/order.pyx:4198-4200`) and is live-published from two sources: the IBKR adapter's `openOrder` callback regenerates it whenever `total_qty != nautilus_order.quantity` **or** a price/trigger differs (`adapters/interactive_brokers/execution.py:968-978`), and reconciliation emits it whenever `report.quantity != order.quantity` (`live/execution_engine.py:1812-1829`, `_should_update` at `:1893-1895`, called per reconciled order at `:1192-1193`). Downsize 100→50: fills of 50 give `cum_qty="50" order_qty="100"` — a complete order recorded as still working, and the entry never prunes. Upsize 100→200: the fill of 100 trips the completion prune, the next fill re-seeds at zero. The story's own `test_an_order_updated_event_produces_no_record` pins the silence and stays green under both. Decide: add an `OrderUpdated` dispatch entry that refreshes `order_qty` (widens the closed set this story deliberately fixed, and reverses a test-pinned decision), or defer to Story 3.4 with the staleness disclosed in the docstring. Found by two layers independently. **DECIDED 2026-08-30 — refresh silently, emit no record:** add an `OrderUpdated` dispatch entry that refreshes `entry["order_qty"]` and logs nothing, on the same harvest-only precedent as `_harvest_initialized`. Rationale: fixes completion detection in both directions, adds no eighth emitted name and so needs no AR41 namespace ruling, and `test_an_order_updated_event_produces_no_record` stays green *and* still correct rather than being reversed. Now a `[Review][Patch]`.
- [x] [Review][Decision] **The observer subscribes after the node is already running and reconciling, so a mid-flight order's first record is knowingly wrong and unmarked** [src/core/live_session_runner.py:507 vs :561-563] — `_phase_node_connect` launches `node.run_async()` at `:507` and the loop is pumped through `_phase_gate_account`; the observer only subscribes at `:563`, after gate/reconcile/warmup. Reconciliation is on by default (`live/config.py:166`) and `live_node_builder.py:408-410` does not override it. A reconciliation-sourced order never publishes `OrderInitialized` at all (`live/execution_engine.py:1758-1762` builds it via `OrderUnpacker.from_init` and never calls `_handle_event`), so for an order already working at attach time the observer's first sight can be an `OrderFilled` whose record carries a partial `cum_qty`, no `order_qty`, and — if the fill is live rather than reconciled — no `reconciliation` flag either. It is byte-identical to a first fill on a fresh order. The docstring's disclosed residual (b) frames this as *"`cum_qty` resets across a process restart — Story 3.4/Epic 4's working-order resume is when that matters"*, which understates it: the wrong `cum_qty` is emitted **within this session**, on this story's own records. Decide: mark first-sight-without-init records explicitly, move the subscribe earlier, or accept and correct the residual's wording. **DECIDED 2026-08-30 — mark the record:** when `_log_filled` finds no seeded entry (`order_qty is None`), emit an explicit `order_qty_unknown=True`. Rationale: makes the record self-describing, which is exactly what AC #5 asks of it, and costs no runner diff — moving the subscribe earlier would break this story's zero-runner-diff evidence contract and touch a module on two hand-maintained guard lists. Residual (b)'s wording is corrected in the same edit. Now a `[Review][Patch]`.
- [x] [Review][Decision] **`venue_reason` is logged verbatim and is an unscanned channel for the identifier NFR26 fences** [src/core/live_order_path.py:465] — the anti-field scan proves no record carries a *key* named `account_id`; it says nothing about values. IBKR rejection text routinely embeds the account code, and AC #3 mandates `str(event.reason)` with *"no rewording, truncation, or classification"*. The test class docstring escalates the claim to *"this story pins the stricter never-logged-at-all"*, which a key-name scan does not establish. Same exposure via `_log_denied`'s `reason`. Decide: does NFR26 target the field or the identifier? If the identifier, AC #3's verbatim mandate and NFR26 are in direct tension and one must give. **Checked during review:** NFR26's normative form is value-level, not field-level — *"any account identifier **in it** is masked to its last 3 characters"*, where "it" is a rendered message (`epics.md:549`, `:650`). So the tension is real, not a misreading. `mask_account` (`src/core/live_gate.py:117`) is a whole-value masker; masking a token *inside* free text is machinery that does not exist. **DECIDED 2026-08-30 — keep verbatim, escalate the conflict:** no behaviour change. AC #3 is unambiguous, and NFR26's own acceptance criteria are scoped to gate and account-verification messages this system renders, not to a venue's own text. Two follow-ups, both `[Review][Patch]`: narrow the `TestNoRecordEverCarriesAnAccountId` docstring's *"this story pins the stricter never-logged-at-all"* to what a key-name scan actually proves, and record the AC-vs-NFR conflict for an epic-level ruling rather than settling it inside a story.

- [x] [Review][Patch] **Cancel/expire with zero prior fills deletes the harvested `order_qty`, breaking AC #5's finality clause in the exact race the retention rule was written for** [src/core/live_order_path.py:531-539] — the entry holds two pieces of state and `_prune_if_no_fills` deletes both, but the retention rule (Task 3.2) is argued purely on `cum_qty` truthfulness. Sequence `OrderInitialized(100) → OrderSubmitted → OrderCanceled → OrderFilled(100)`: at the cancel `cum_qty == 0` so the entry goes, then `_log_filled`'s `setdefault` at `:483` re-seeds with `order_qty: None`. The record shows `cum_qty="100"` with no `order_qty`, the completion test at `:504` can never fire, and the entry survives to session end. This is the *common* form of the race — a working order filling before the cancel reaches the venue usually has no prior fill — so the guard covers the rarer sub-case and misses the likelier one. The one test, `test_a_late_fill_across_the_cancel_ack_still_accumulates`, always fills *before* the cancel. Fix: stop pruning on cancel/expire (memory cost identical to the already-disclosed residual (a)), or retain `order_qty` alone. Found by all three layers.
- [x] [Review][Patch] **No record carries `ts_event`, which Task 2.2 mandates on every one** [src/core/live_order_path.py:445-450, 461-466, 485-495, 519-523, 547-552] — Task 2.2 (marked `[x]`) reads *"every record also carries `strategy_id` … and `ts_event` as the venue-side nanos"*. No field builder emits it; `grep -n ts_event src/core/live_order_path.py` returns only `note_bar`'s `bar.ts_event` and `_log_submitted`'s latency subtraction. No test asserts it. The rebuttal that structlog timestamps it does not hold: `src/utils/logging.py:54` is `TimeStamper(fmt="iso")`, i.e. host log-write time, not the venue instant — precisely the distinction the story built the `reconciliation` flag to preserve.
- [x] [Review][Patch] **Four of the five accumulator prune statements cannot be killed by any test** [src/core/live_order_path.py:471, 505, 539, 556] — no test in the repository reads `observer._orders`, and no test sends an event *after* a prune point for the same `client_order_id`. Deleting the `pop` in `_log_rejected` (`:471`), the `pop` in `_log_denied` (`:556`), the `del` in the fill-completion branch (`:505`), or the `del` inside `_prune_if_no_fills` (`:539`) each leaves the suite green. Only the *retention* half of `_prune_if_no_fills` is guarded (by M10). The commit message's "eleven scripted mutations … all killed" is accurate for what was scripted; the accumulator's memory-hygiene rules — the diff's most subtle logic, and the subject of a full docstring paragraph — were not among the mutants. Found by two layers.
- [x] [Review][Patch] **`_log_filled` mutates the accumulator before emitting, and the containment does not roll back** [src/core/live_order_path.py:483-503] — `entry["cum_qty"]` is incremented at `:484`, then twelve attribute reads build the field dict, then the record is emitted at `:503`. Any raise in between is caught by `handle_order_event`, which logs `order.observer_failed` and returns, leaving the counter permanently advanced with no record accounting for it; the next successful fill then logs an inflated `cum_qty` and can trip the completion prune early. Fix is a reorder: compute the new total into a local, build fields from it, log, then commit to the entry. The existing containment test passes an attribute-less stub that raises at `:482` — before the mutation — so it cannot see this window. Found by two layers.
...
- [x] [Review][Patch] **Over-fill is reported as exact completion, and the prune then makes `cum_qty` non-monotonic** [src/core/live_order_path.py:504-505] — the branch is `>=` with a single outcome, so `cum_qty > order_qty` emits a record shaped identically to an exact completion with no marker that the order over-filled. Because the entry is then deleted, a further fill re-seeds at zero and reports a `cum_qty` *lower* than the previous record's — contradicting the module docstring's own *"the rising `cum_qty` series is the representation"*. Reachable from the duplicate-fill and stale-`order_qty` findings above as well as from a genuine venue over-fill, which Nautilus tolerates rather than rejects (`model/orders/base.pyx:1118-1120`).
...
### Review Patches Applied (2026-08-30)

13 of 15 `[Review][Patch]` items applied, plus all 4 resolved `[Review][Decision]` items. Two left
open and marked `[NOT APPLIED]`: the **Task 7.2 clause matrix** (substantial authored analysis, and
...
- The accumulator is **never pruned**. All four prune sites are gone. This followed from the
  duplicate-fill decision rather than being chosen for its own sake: `trade_ids` de-duplication only
  works if the entry survives, and the old completion prune destroyed exactly the memory that
  detects the commonest case (a single-fill order redelivered once). It simultaneously fixes the
  cancel-with-no-prior-fills hole, which deleted the harvested `order_qty` in the common form of
  the fill-crosses-the-cancel-ack race. Cost: one entry per `client_order_id` per session — the
  `_last_bar` shape residual (a) already disclosed for a subset.
- `_OrderAccumulator` is a `TypedDict` carrying `cum_qty`, `order_qty`, `trade_ids`.
- `_log_filled` de-duplicates on `trade_id`, emits `duplicate=True` without accumulating, and
  **commits the accumulator after a successful emit** rather than before.
- Three new markers make a record state what it previously left the reader to assume:
  `duplicate`, `order_qty_unknown`, `over_fill`.
- `OrderUpdated` joins the dispatch as a **silent** harvest refreshing `order_qty`, on the
  `_harvest_initialized` precedent — no eighth AR41 name, and the Task 2.6 silence pin stays true.
- `ts_event` added to all seven lifecycle records (Task 2.2's unmet mandate).
- `order.observer_failed` now carries `event_type` and `client_order_id`, both via `getattr` so the
  diagnostic cannot raise on the malformed event that reached it.

**Test changes** — `tests/component/core/test_live_order_path.py`, 49 → 79 cases:

- `TestEveryDispatchedRecordNameIsPinned` derives the emitted-name set by driving every entry in the
  observer's own `_dispatch` map, closing the one-directional hole in the old membership pin.
- New: duplicate-fill (3), `OrderUpdated` (3), retention (3), partial-knowledge/over-fill (2),
  `ts_event` (7 parametrized), severity (7 parametrized), observer-failure diagnostics (2),
  containment-leaves-no-phantom-total (1).
- Strengthened: `SUBMITTED_EVENT` literal pin; AC #2's mandatory triple on `order.expired`;
  `instrument_id` on `order.canceled`; a missing `len(records) == 1`; a new
  `venue_order_id`-present case for expiry.

**A fixture defect found while patching, not by any review layer.** `TestEventStubs.order_filled`
derives `trade_id` from the `client_order_id`, so every stub fill on one order shares an execution
id unless overridden. All three partial-fill tests — the ones discharging AC #1's central
representation decision — were therefore constructing an event stream **no venue can produce**: two
distinct executions with one execution id. The new de-duplication rejected the second fill and the
tests went red, which is how it surfaced. Fixed by passing distinct `TradeId`s. M7 ("reset `cum_qty`
on every fill") was killed by these tests, so the mutation was being killed by a fixture that could
not occur in production.

**Docstring/rules corrections** (claims that were false as written, now narrowed to what is
actually guaranteed):

- `EMITTED_ORDER_EVENTS` was documented as "every event name the observer can emit" — false on the
  day it shipped, since `order.observer_failed` is emitted twice and is not in it. Now scoped to
  order-*lifecycle* records, with diagnostic/boundary records named as deliberately outside.
- `TestNoRecordEverCarriesAnAccountId` claimed to pin "the stricter never-logged-at-all". It
  inspects key names and cannot pin values. Narrowed, with the AC #3 / NFR26 conflict recorded in
  `_log_rejected`'s docstring and escalated rather than settled in a story.
- CLAUDE.md: the Membership-pinned-lists entry carried the false guarantee into project rules. Now
  states the general lesson — **an exact-set pin against hand-written constants is
  one-directional; pin against the code that emits, not a second hand-written list**. Separately,
  `_STDLIB_AND_FIRST_PARTY` added to the enumerated guard lists, since Story 3.3's
  `from decimal import Decimal` tripped it and the story's categorical "zero guard-list edits" claim
  was false.

**Gates after patching**: `make format` 491 files unchanged · `make lint` clean · `make typecheck`
clean, 104 files · unit **2419** (unchanged) · component **1409/16 skipped** (was 1378/16, +31) ·
integration `--forked` **281/2 skipped** (unchanged) · e2e untouched · Epic 1 sweep **43 tests
passed, 40/40 acceptance criteria** — both numbers recorded with their units, since conflating them
is what produced a withdrawn finding in this review.

**Size, disclosed per CLAUDE.md D4** (measured on one consistent AST counter, so these are deltas
from the pre-patch tree rather than a comparison against the Dev Agent Record's own figures, which
used a slightly different method): `live_order_path.py` **556 → 694 raw lines**, module executable
statements **151 → 162**, `OrderEventObserver` **83 → 88** statements. Raw lines are well over the
500 figure the cap names; the growth is almost entirely docstring recording measured framework
behaviour — the D4 case for measuring statements, not lines. The observer class stays under the
100-statement cap but the margin narrowed from 17 to 12. Not split, per the guard-list-rot
Anti-Pattern.

**Live verification still has not run.** Nothing in this review changes that: Task 6 remains
outstanding, and AC #1's "as they arrive from the venue" and AC #5's real-fill clause are still
discharged only on the automated tiers.
