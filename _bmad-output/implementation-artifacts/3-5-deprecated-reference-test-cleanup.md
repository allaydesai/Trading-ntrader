...
- [ ] Task 1: Delete the stub and verify no live references (AC: #1, #5)
  - [ ] 1.1 `git rm tests/integration/core/test_aapl_2018_reference_comparison.py`
  - [ ] 1.2 `grep -rn "test_aapl_2018_reference_comparison" src/ tests/ scripts/ Makefile pyproject.toml` — confirm zero live-code matches.
  - [ ] 1.3 If any live-code match surfaces, surface in Review Findings before proceeding (no live caller is expected).

- [ ] Task 2: Update `deferred-work.md` (AC: #2)
  - [ ] 2.1 Remove the C1 and C2 entries from the `## Deferred from: code review of 3-3-...` section.
  - [ ] 2.2 Confirm C3–C11 in that section are untouched.
  - [ ] 2.3 Confirm the file header and section structure are preserved exactly.

- [ ] Task 3: Update `scripts/verify_aapl_2018_reference.py` docstring (AC: #3)
  - [ ] 3.1 Read the existing docstring/comment at the live reference site.
  - [ ] 3.2 Replace `test_aapl_2018_reference_comparison.py` with `test_aapl_2018_ibkr_vs_firstrate.py` in any prose reference. If the original sentence framed the deleted file as the "integration test entrypoint", reframe to point at the live test.
  - [ ] 3.3 Verify the script still imports and runs (`uv run python scripts/verify_aapl_2018_reference.py --help` exits 0).

- [ ] Task 4: Quality gates (AC: #4)
  - [ ] 4.1 `make format && make lint && make typecheck` — clean.
  - [ ] 4.2 `make test-unit` — zero regressions (851 baseline).
  - [ ] 4.3 `make test-component` — zero regressions (621 baseline).
  - [ ] 4.4 `make test-integration` — runs to completion. Confirm Story 3.4's `test_aapl_2018_ibkr_vs_firstrate.py` is collected (3 tests, 1 design-mandated FAIL per AC #6, 2 PASS) when `IBKR_AVAILABLE=1 E2E_CATALOG_AVAILABLE=1` is set; skips cleanly when not. Note any new failures inline.
