...
- D9: `_bar_count_field_for_timeframe` silently falls back to `bar_count_daily` for unknown aggregations (`src/services/firstrate/import_service.py:44`) — mirrored in `_upsert_metadata` (`:545`). CLI `TIMEFRAME_MAP` currently only emits DAY/HOUR/MINUTE so the fallback is unreachable in production, but a direct `import_service.import_directory()` call with a novel aggregation (e.g., `"1-SECOND-LAST"`, `"1-WEEK-LAST"`) would silently misroute to the daily bar_count and could trigger a false skip. Raise `ValueError` on unknown aggregation in both the helper and `_upsert_metadata` when a non-daily aggregation is added.
