...
class BacktestOrchestrator:
    ...
    async def execute_multi(
        self,
        request: BacktestRequest,
        instrument_bars: list[tuple[Instrument, list[Bar]]],
    ) -> tuple[BacktestResult, UUID | None]:
        """Execute a multi-instrument backtest in a single BacktestEngine instance.

        Follows the strict Nautilus engine-setup order:
          1. ``add_venue`` once per unique venue
          2. ``add_instrument`` once per instrument
          3. ``add_data`` once per bars list
          4. ``add_strategy`` (primary = first instrument)
          5. ``run``

        The engine is single-use (CLAUDE.md Gotcha #4): callers MUST create a
        fresh orchestrator per run and call ``dispose()`` in a finally block.

        Args:
            request: Unified backtest request.
            instrument_bars: List of ``(instrument, bars)`` pairs. The first
                pair's instrument is the strategy's primary instrument.

        Returns:
            ``(BacktestResult, run_id if persisted else None)``.

        Raises:
            ValueError: If ``instrument_bars`` is empty or strategy cannot be
                created.
        """
        execution_start_time = time.time()
        run_id = uuid4() if request.persist else None
        ...
        if not instrument_bars:
            raise ValueError("No instrument_bars provided for multi-instrument backtest")

        primary_instrument, primary_bars = instrument_bars[0]

        try:
            self._setup_engine_multi(request, instrument_bars)

            strategy = self._create_strategy_multi(request, primary_bars, primary_instrument)
            if strategy is None:
                raise ValueError(f"Failed to create strategy: {request.strategy_type}")
            ...
            if request.persist and run_id:
                ...
                logger.info(
                    "Multi-instrument backtest completed and persisted",
                    run_id=str(run_id),
                    strategy=request.strategy_type,
                    symbols=[inst.id.symbol.value for inst, _ in instrument_bars],
                )
    ...
    def _setup_engine_multi(
        ...
        """Configure the engine for a multi-instrument run (strict Nautilus order).

        Unique venues are deduplicated by ``venue.value`` to avoid the
        "duplicate venue" error from the engine.
        """
        ...
        for instrument, _ in instrument_bars:
            ...
            unique_venues.setdefault(str(venue), venue)

        first_venue: Venue | None = None
    ...
    def _create_strategy_multi(
        self,
        request: BacktestRequest,
        bars: list[Bar],
        instrument: Instrument,
    ) -> Strategy:
        """Create the strategy using the primary instrument (first in list).

        Story 3.1 passes only the primary instrument's id + bar_type to the
        strategy config. Multi-instrument strategies with per-instrument
        configs are a future enhancement (tracked separately).
        """
        return self._create_strategy(request, bars, instrument)
