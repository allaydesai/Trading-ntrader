...
async def _load_chart_bars(backtest: Any) -> list:
    """Load the OHLCV bars an indicator overlay should be computed against.
    ...
    - For a named-catalog run (``config_snapshot.catalog_name`` or a
      ...
      symbol and returned nothing, leaving the overlay empty on real data.
    """
    cfg = backtest.config_snapshot or {}
    bar_type_spec = cfg.get("bar_type") or "1-DAY-LAST"
    ...
    if catalog_name:
        ...
        if not base_path:
            raise CatalogError(
                ...
                f"'{catalog_name}' for indicator overlay."
            )
