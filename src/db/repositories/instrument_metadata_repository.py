"""Async repository for the instrument_metadata cache table.

Used by web API endpoints via FastAPI dependency injection. The sync
counterpart for CLI use lives in
``instrument_metadata_repository_sync.py``.
...
class InstrumentMetadataRepository:
    """Async repository for instrument metadata operations.
    ...
    async def upsert(self, metadata: InstrumentMetadata) -> InstrumentMetadata:
        ...
        """
        try:
            existing = await self.get_by_ticker(metadata.ticker)
            if existing:
    ...
    async def get_by_ticker(self, ticker: str) -> Optional[InstrumentMetadata]:
