...
def cmd_load(args):
    ...
    if args.all:
        ...
        if not var_specs:
            print(
                json.dumps({"error": "Either --vars or --all must be specified"}), file=sys.stderr
            )
            sys.exit(1)
        result = {}
...
def cmd_resolve_defaults(args):
    ...
    if not module_def:
        print(
            json.dumps({"error": f"Failed to parse module.yaml at: {target_yaml_path}"}),
            file=sys.stderr,
        )
        sys.exit(1)
...
def cmd_write(args):
    ...
    if not project_root:
        if args.project_root:
            ...
            print(
                json.dumps({"error": "Project root not found and --project-root not provided"}),
                file=sys.stderr,
            )
            sys.exit(1)
